...
- **One-command test gate: `tests/run-all.sh`.** Discovers and runs every `tests/test-*.sh`, propagates failure, and prints a per-file roll-up. CI-ready, and the exact step the fresh-clone verification uses. Current suite: 20 assertions across 2 files, all green.
